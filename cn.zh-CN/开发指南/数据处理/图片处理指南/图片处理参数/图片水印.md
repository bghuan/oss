...
本文示例使用的Bucket为张家口地域名为image-demo-oss-zhangjiakou的Bucket，图片外网访问地址为[https://image-demo-oss-zhangjiakou.oss-cn-zhangjiakou.aliyuncs.com/](https://image-demo-oss-zhangjiakou.oss-cn-zhangjiakou.aliyuncs.com/)。
...
-   为原图添加多个水印
    ...
    当需要添加多个水印时，需使用正斜线将不同的水印操作隔开。示例图片包括4个不同的文本水印和5个图片水印，图片水印使用到了3张不同的水印图片。
